...
api = Namespace(
    'auth-jwt', description="Authorization JWT API v1 (Not Implemented)")

auth_model = api.model(
    'auth_jwt', {
        "message":
        fields.String(
            required=False, description="Message login for [username]"),
        "access_token":
        fields.String(required=False, description="The JWT Hash Token"),
        "refresh_token":
        fields.String(
            required=False, description="The JWT hash Refresh Token")
    })
